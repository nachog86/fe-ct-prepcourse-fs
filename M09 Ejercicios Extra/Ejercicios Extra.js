...
function asAmirror(frase) {
   ...
   for(let i=0; i< frase.length; i++){
      nuevaFrase.unshift(frase[i])
   }
   return nuevaFrase.join('').split(' ').reverse().join(' ');
}
...
function capicua(numero) {
   ...
   let resultado = parseInt(nuevoNumero);
   if(numero === resultado){
      return "Es capicua"
   }
...
function deleteAbc(string) {
   ...
   // Tu código:
   let nuevoString = string.split('').filter((e)=> e!== 'a' && e!== 'b' && e!=='c').join('')
   return nuevoString;
}
